...
| `[BR99]` | Beale, J. C.; Reitz, R. D. "Modeling spray atomization with the Kelvin-Helmholtz/Rayleigh-Taylor hybrid model." *Atomization and Sprays*, 9, 1999, pp. 623–650. | DOI:10.1615/AtomizSpr.v9.i6.40 | yes (2026-07-15, source PDF `KHRT-hybrid.pdf`; RT eqs 8–11 — A14. `Reitz-KHRT1999.pdf` is a 0-char scan. **Note 2026-08-03:** BR99 restates the KH `Λ` fit with **`0.865`** where `[Reitz87]` Eq. 4 prints **`0.87`**; production follows the origin paper — see `breakup/khrt-e2e/INFO.md`) |
| `[Ray78]` | Rayleigh, Lord (J. W. Strutt). "On the Instability of Jets." *Proc. London Math. Soc.*, s1-10, 1878, pp. 4–13. | DOI:10.1112/plms/s1-10.1.4 | **not needed as a source** — the result used is the classical inviscid capillary dispersion relation `ω² = (σ/ρa³)·x(1−x²)I₁(x)/I₀(x)`, which `test_kh_rayleigh_limit` **solves from scratch** (Bessel series + golden-section) rather than transcribing: `x*=0.697019`, `λ/a=9.01437`, `ω*=0.343339`. `[Reitz87]` p.318 states the required reduction in words ("the maximum growth rate occurs at Λ = 9.02a"), which is the anchor being checked |
