...
│   └── drag-stokes/ temp-relax/ body-force/ conv-nu/ vie-plait/ swirl-wedge/   # e2e cases
...
│                                 # refusals/{p2t,zgr,lk-d2law,properties-*,*-token,evaporation-leb,tab-method,gas-order,out-file,ode-solver,dopri5} (setup-refusal gates)
...
|----------|----------|--------|
| `standard` | Drag (A) and temperature (B) unit families; drag-stokes, temp-relax, body-force, conv-nu, vie-plait, swirl-wedge e2e | **GREEN** |
...
Current gate: **80/80** in a serial build (81 registered: 53 e2e + 28 unit, `self_test` and `registry-docs` among the latter; `drag-stokes-dopri5` is registered DISABLED until the OSlo fix for ledger O25 lands); `USE_MPI=ON` registers 7 more `mpi-*` cases, 87/87.
